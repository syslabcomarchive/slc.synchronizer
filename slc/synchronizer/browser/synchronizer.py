...
class Synchronizer(BrowserView):
    ...
    def __call__(self):
        ...
        if (R.has_key('savecredentials')
            and R.get('savecredentials', '') != ''):
        ...
        if R.has_key('form.button.Synchronize'):
            ...
            for ob in obs:
                ...
                syncstatus = self.syncObject(
                    extractor.portal_type(),
                    extractor.data(),
                    remote_uid=ob.UID(),
                    translation_reference_uid=self._get_trans(ob))
                if syncstatus[0] == 0:
                    log("%s - URL: %s" %
                        (syncstatus[1], syncstatus[2]), 'info')
                elif syncstatus[0] == 1:
                    log("%s - URL: %s" %
                        (syncstatus[1], syncstatus[2]), 'warning')
                else:
                    log("%s - URL: %s" %
                        (syncstatus[1], syncstatus[2]), 'error')
            ...
            if R.has_key('credentials'):
                ...
                log("No credentials selected. Nothing has been deleted",
                    'warning')
    ...
    def _save_credentials(self):
        """ save entered credentials into the storage, if the checkbox is
        checked
        """
    ...
    def default_credentials(self):
        """ called by the form it checks if there is a credentials field
        in the request.
        if not, it checks if we have a default credential key as annotation
        on the object.
        """
        R = self.context.REQUEST
    ...
    def _generate_target_url(self):
        ...
        targeturl = "http://%s:%s@%s/synchronize_receiver" % (
            login, password, server[7:])
    ...
    def credentials(self):
        """ read existing credentials from the storage and display login
        and url as key
        """
        storage = queryUtility(IAccessStorage)
        for cred in storage:
        ...
        for field in fields:
            ...
            if ftype == 'Products.Archetypes.Field.ReferenceField':
                for item in field.getAccessor(self.context)():
                    refs.append(
                        (item, pw.getCatalogVariablesFor(item).get(
                            'review_state', ''), fname))
        return refs
    ...
    def syncObject(self, portal_type,
        ...
        """
        logger.info("Synchronizing %s, remote_uid %s, translation %s" %
                    (portal_type, remote_uid, translation_reference_uid))
        try:
            syncstat = self.rpc.syncObject(portal_type, data,
                self.getSiteId(), remote_uid, translation_reference_uid)
        except Exception, e:
            return (2, str(e), '')
        # if syncing was successful, remember the used credentials on
        # this object
        server, login, password = self._get_credentials()
        if server and login:
