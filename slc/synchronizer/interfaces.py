...
class IReceiver(Interface):
    """ Interface for a component that receives data from a remote site to
    add objects to this site.
    """
    ...
    def getSyncStatus(site_id, remote_uid):
        """ looks up the remote uid in the local utility to get the local uid.
            ...
            returns the modification date of the referenced object or None,
            if no such object exists
            as well as the link of the object
        """

    def syncObject(portal_type, data={}, site_id=None, remote_uid=None,
...
class IUIDMappingStorage(Interface):
    """ A storage for uids which maps a remote uid and a remote site
    identifier to a local uid registered as a local utility.
    """
...
class IDataExtractor(Interface):
    """ Interface for an adapter that extracts the content of object and
    returns it as a key-value-mapping.
    """
